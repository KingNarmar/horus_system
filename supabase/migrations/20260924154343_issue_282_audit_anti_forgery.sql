...
-- Fleet license-document RPCs already write their own semantic audit events.
-- They insert a document file before optionally updating the Fleet asset's
-- license expiry. Mark that transaction so the generic asset trigger does not
-- emit a duplicate tractor_head_updated/trailer_updated audit event.
...
-- Driver Settlement items are inserted in a separate client request. The audit
-- snapshot intentionally excludes items_count rather than mutating an audit row
-- after creation with a value that is not atomic with the settlement insert.
