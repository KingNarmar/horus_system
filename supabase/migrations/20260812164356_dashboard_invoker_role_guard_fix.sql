-- H.O.R.U.S System — Issue #29 Dashboard invoker role guard fix
--
-- SECURITY INVOKER intentionally keeps table RLS active. The initial function
-- called a private-schema helper directly, which authenticated does not have
-- schema USAGE for. Resolve the dashboard role gate through company_users under
-- its existing RLS instead of widening access to the private schema.
