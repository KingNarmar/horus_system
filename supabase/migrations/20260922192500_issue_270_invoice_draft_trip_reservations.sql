-- Issue #270: prevent the same Trip from being reserved by multiple active
-- invoices while preserving the existing Draft -> Issued lifecycle.
--
-- Draft reservations live in invoice_lines. trips.invoice_id remains NULL until
-- the invoice is issued because trips_invoice_status_link_check requires it.
...
BEGIN
  IF EXISTS (
    ...
    JOIN public.invoices AS invoice_row
      ON invoice_row.company_id = line_row.company_id
     AND invoice_row.id = line_row.invoice_id
    WHERE invoice_row.status <> 'cancelled'
    GROUP BY line_row.company_id, line_row.trip_id
    HAVING pg_catalog.count(DISTINCT line_row.invoice_id) > 1
  ) THEN
    RAISE EXCEPTION
      'Invoice draft reservation hardening stopped: duplicate active Trip reservations exist.';
  END IF;
END
$$;
...
BEGIN
  ...
  WHERE trip_row.company_id = p_company_id
    ...
    AND NOT EXISTS (
      SELECT 1
      FROM public.invoice_lines AS reserved_line
      JOIN public.invoices AS reserved_invoice
        ON reserved_invoice.company_id = reserved_line.company_id
       AND reserved_invoice.id = reserved_line.invoice_id
      WHERE reserved_line.company_id = trip_row.company_id
        AND reserved_line.trip_id = trip_row.id
        AND reserved_invoice.status <> 'cancelled'
    )
...
BEGIN
  ...
  -- Lock every requested Trip in one deterministic order so concurrent grouped
  -- draft creation cannot reserve the same Trips or deadlock on opposite input
  -- ordering.
  FOR v_trip IN
    SELECT
  ...
  LOOP
    ...
    IF EXISTS (
      ...
      JOIN public.invoices AS reserved_invoice
        ON reserved_invoice.company_id = reserved_line.company_id
       AND reserved_invoice.id = reserved_line.invoice_id
      WHERE reserved_line.company_id = p_company_id
        AND reserved_line.trip_id = v_trip.id
        AND reserved_invoice.status <> 'cancelled'
        AND (
          p_invoice_id IS NULL
          OR reserved_invoice.id <> p_invoice_id
        )
    ) THEN
      RAISE EXCEPTION USING
        ERRCODE = 'P2617',
  ...
  ELSE
    ...
    DELETE FROM public.invoice_lines AS line_row
    WHERE line_row.company_id = p_company_id
      AND line_row.invoice_id = p_invoice_id;
  ...
  PERFORM private.write_invoice_audit_event(
    ...
    CASE
      WHEN v_is_create THEN 'invoice_created'
