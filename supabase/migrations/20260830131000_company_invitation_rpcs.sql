...
BEGIN
  ...
  PERFORM private.write_audit_event(
    ...
    pg_catalog.jsonb_build_object(
      'expires_at', v_invitation.expires_at - private.company_invitation_ttl()
    ),
    pg_catalog.jsonb_build_object(
      'expires_at', v_invitation.expires_at
    ),
...
BEGIN
  ...
  SELECT invitation.*, company_row.name
  INTO v_invitation, v_company_name
  FROM public.company_invitations invitation
  JOIN public.companies company_row
    ON company_row.id = invitation.company_id
   AND company_row.is_active = true
...
BEGIN
  ...
  RETURN QUERY
  SELECT v_membership.id, v_membership.company_id, v_membership.role;
END;
$$;
