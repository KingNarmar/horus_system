...
void main() {
  test('provides focused English business timezone readiness copy', () {
    final strings = BusinessTimezoneLocalizations.forLocale(const Locale('en'));
    ...
    expect(
      strings.configurationRequired.toLowerCase(),
      isNot(contains('currency')),
    );
  });

  test('provides focused Arabic business timezone readiness copy', () {
    final strings = BusinessTimezoneLocalizations.forLocale(const Locale('ar'));
