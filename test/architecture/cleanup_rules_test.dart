...
void main() {
  group('cleanup architecture rules', () {
    test('localized server errors do not expose raw failure messages', () {
      final content = _read('lib/core/localization/app_localizations_extension.dart');

      expect(content, isNot(contains('FailureCodes.serverError =>\n        failure.message')));
      expect(content, contains('FailureCodes.serverError => _genericServerErrorMessage'));
      expect(content, contains('failure is ServerFailure'));
    });

    test('audited repositories use semantic descriptions instead of English sentences', () {
      ...
      for (final path in repositoryPaths) {
        ...
        for (final snippet in forbiddenSnippets) {
          expect(
            ...
            reason: '$path must use semantic audit descriptions, not "$snippet".',
          );
        }
      }
    });
