...
class DriversTable extends StatelessWidget {
  ...
  const DriversTable({required this.drivers, required this.canManageDrivers, required this.onViewDetails, required this.onEdit, required this.onDeactivate, required this.onReactivate, super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Card(child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: ConstrainedBox(constraints: const BoxConstraints(minWidth: AppSizes.desktopMinWidth), child: DataTable(columns: [DataColumn(label: Text(l10n.driverNameLabel)), DataColumn(label: Text(l10n.phoneLabel)), DataColumn(label: Text(l10n.statusHeader)), DataColumn(label: Text(l10n.actionsHeader))], rows: drivers.map((driver) {
      return DataRow(cells: [DataCell(Text(driver.fullName)), DataCell(Text(driver.phone ?? l10n.emptyValue)), DataCell(Text(l10n.driverStatusLabel(driver.status))), DataCell(_DriverActions(driver: driver, canManageDrivers: canManageDrivers, isActionInProgress: pendingActionDriverId == driver.id, onViewDetails: onViewDetails, onEdit: onEdit, onDeactivate: onDeactivate, onReactivate: onReactivate))]);
    }).toList()))));
  }
}
...
class _DriverActions extends StatelessWidget {
  ...
  const _DriverActions({required this.driver, required this.canManageDrivers, required this.isActionInProgress, required this.onViewDetails, required this.onEdit, required this.onDeactivate, required this.onReactivate});

  @override
  Widget build(BuildContext context) {
    final l10n = context.l10n;
    final active = driver.status.isActive;
    return Row(mainAxisSize: MainAxisSize.min, children: [IconButton(tooltip: l10n.viewDriverDetails, onPressed: () => onViewDetails(driver), icon: const Icon(AppIcons.view)), if (canManageDrivers) ...[IconButton(tooltip: l10n.editDriverButton, onPressed: isActionInProgress ? null : () => onEdit(driver), icon: const Icon(AppIcons.edit)), IconButton(tooltip: active ? l10n.deactivateDriverButton : l10n.reactivateDriverButton, onPressed: isActionInProgress ? null : () => _handle(context, active), icon: _ActionIcon(isLoading: isActionInProgress, icon: active ? AppIcons.deactivate : AppIcons.reactivate))]]);
  }
