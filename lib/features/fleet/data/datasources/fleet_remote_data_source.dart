...
class SupabaseFleetRemoteDataSource implements FleetRemoteDataSource {
  ...
  Future<List<TractorHeadModel>> getTractorHeads({
    ...
    final rows = await client
        .from(TractorHeadDbFields.tableName)
        .select(TractorHeadDbFields.allColumns)
        .eq(DbCommonFields.companyId, companyId)
        .order(TractorHeadDbFields.plateNumber);
    return rows
        .map((row) => TractorHeadModel.fromMap(Map<String, dynamic>.from(row)))
        .toList();
  }
  ...
  Future<List<TrailerModel>> getTrailers({required String companyId}) async {
    final rows = await client
        .from(TrailerDbFields.tableName)
        .select(TrailerDbFields.allColumns)
        .eq(DbCommonFields.companyId, companyId)
        .order(TrailerDbFields.plateNumber);
    return rows
        .map((row) => TrailerModel.fromMap(Map<String, dynamic>.from(row)))
        .toList();
  }
